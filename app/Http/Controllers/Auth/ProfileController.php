...
class ProfileController extends Controller
{
    ...
    protected function sendToken()
    {
        ...
        $message = $sendMailResult
            ? 'Письмо удачно отправлено. Проверьте свою почту.'
            : 'Не удалось отправить письмо.';
        session()->put('resent', $message);
    ...
    protected function showSettings()
    {
        ...
        return view('auth.profile',
            compact(['userEmail', 'userUnconfirmedEmail']));
    }
    ...
    {   /*запись в сесию присланного Email-а чтобы потом закинуть в поле.
        ...
        if ($sendMailResult) {
            ...
            session()->put('send_email_message',
                'Письмо удачно отправлено. Проверьте свою почту.');
        } else {
    ...
    private function sendMail(User $user, string $token)
    {
        try {
            ...
            Mail::to($user->unconfirmed_email)
                ->send(new VerifyChangeEmail(env('APP_URL')
                                             . "/verification/$user->id/$token"));
    ...
    protected function verificationChangeEmail(int $id, string $token)
    {
        ...
        if ($user === null) {
            ...
            session()->put('infoMessage',
                'Токен не подходит для данного пользователя!');
